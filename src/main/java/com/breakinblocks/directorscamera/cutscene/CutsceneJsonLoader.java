...
public class CutsceneJsonLoader extends SimpleJsonResourceReloadListener<JsonElement> {
    public CutsceneJsonLoader() {
        super(ExtraCodecs.JSON, FileToIdConverter.json("directorscamera/cutscenes"));
    }

    @SubscribeEvent
    public static void onAddReloadListeners(AddServerReloadListenersEvent event) {
        event.addListener(DirectorsCamera.id("cutscenes"), new CutsceneJsonLoader());
    }
    ...
    protected void apply(Map<Identifier, JsonElement> map, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        map.forEach((id, json) -> {
            try {
                ...
                DirectorsCamera.LOGGER.error("Failed to load cutscene {}", id, e);
            }
        });
        CutsceneRegistry.setDataDefinitions(definitions);
        DirectorsCamera.LOGGER.info("Loaded {} cutscene definitions", definitions.size());
    }
