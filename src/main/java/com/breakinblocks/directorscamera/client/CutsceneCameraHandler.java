...
import com.breakinblocks.directorscamera.cutscene.StopMode;
import com.breakinblocks.directorscamera.mixin.MouseHandlerAccessor;
import com.breakinblocks.directorscamera.net.CutsceneStatePayload;
...
public final class CutsceneCameraHandler {
    ...
    public static void startCutscene(CutsceneData data) {
        ...
        if (data.keyframes().isEmpty()) {
            DirectorsCamera.LOGGER.warn("Ignoring cutscene {} with no keyframes", data.id());
            return;
        }
        ...
        if (DirectorsCameraConfig.CLIENT.closeScreenOnStart.get() && minecraft.screen != null) {
        ...
        restAlpha = 1.0F;
        restAlphaPrev = 1.0F;
        ClientPacketDistributor.sendToServer(new CutsceneStatePayload(CutsceneStatePayload.STARTED, data.id()));
    ...
    public static void stopCutscene(StopReason reason) {
        ...
        mouse.directorscamera$setAccumulatedDX(0.0);
        mouse.directorscamera$setAccumulatedDY(0.0);
        ...
        ClientPacketDistributor.sendToServer(new CutsceneStatePayload(reason.state, id));
        ClientCutsceneHooks.Holder.get().ended(id, reason.name().toLowerCase());
    }

    public static void nullifyInput(ClientInput input) {
    ...
    public static void onClientTick(ClientTickEvent.Pre event) {
        ...
        while (DirectorsCameraClient.END_CUTSCENE.consumeClick()) {
            endPressed = true;
        }
