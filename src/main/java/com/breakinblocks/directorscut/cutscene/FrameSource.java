...
public interface FrameSource {
    ...
    static FrameSource structure(@Nullable ResourceLocation structureId) {
    ...
    record Structure(@Nullable ResourceLocation structureId) implements FrameSource {
        @Override
        public Optional<CutsceneFrame> resolve(ServerPlayer player) {
            BlockPos pos = player.blockPosition();
            for (StructureStart start : player.serverLevel().structureManager().startsForStructure(new ChunkPos(pos), s -> true)) {
                if (!start.getBoundingBox().isInside(pos)) {
                    continue;
                }
                if (structureId != null) {
                    ResourceLocation id = player.serverLevel().registryAccess().registryOrThrow(Registries.STRUCTURE).getKey(start.getStructure());
