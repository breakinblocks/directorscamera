...
import com.breakinblocks.directorscut.cutscene.CutsceneSound;
...
public final class CutsceneSoundPlayer {
    ...
    public static void fire(CutsceneSound entry, ClientCameraEntity camera) {
        ...
        if (event == null) {
            event = SoundEvent.createVariableRangeEvent(entry.sound());
        }
        SoundInstance instance;
        if (entry.position().isEmpty()) {
            instance = new SimpleSoundInstance(event.getLocation(), entry.category(), entry.volume(), entry.pitch(), SoundInstance.createUnseededRandom(),
        ...
        try {
            ...
            DirectorsCut.LOGGER.warn("Failed to play cutscene sound {}", entry.sound(), e);
            return;
        }
