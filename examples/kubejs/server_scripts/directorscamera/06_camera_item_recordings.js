ServerEvents.commandRegistry(event => {
    ...
    event.register(
        Commands.literal("playrecording").executes(ctx => {
            ...
            const recording = DirectorsCamera.recordingOf(player.mainHandItem);
            if (!recording) {
...
ServerEvents.commandRegistry(event => {
    ...
    event.register(
        Commands.literal("orbittemplate").executes(ctx => {
            ...
            if (stack.id != "directorscamera:directors_camera") {
                player.tell("Hold a Director's Camera");
                return 0;
            }
            const template = DirectorsCamera.cutscene().setDurationSeconds(6);
            template.getPath().addOrbit(player.x, player.y + 2, player.z, 6, 0, 360, 12, true);
            DirectorsCamera.setRecording(stack, template);
