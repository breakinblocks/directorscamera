DirectorsCutEvents.register(event => {
    ...
    reveal.executeAtSecond(5, player => {
        const frame = DirectorsCut.frameOf(player);
        if (frame) {
...
ServerEvents.commandRegistry(event => {
    ...
    event.register(
        Commands.literal("throneroom").executes(ctx => {
            ...
            if (!DirectorsCut.playAnchored(player, "mypack:throne_room", "throne_room")) {
    ...
    event.register(
        Commands.literal("virtualanchor").executes(ctx => {
            const player = ctx.getSource().getPlayerOrException();
            DirectorsCut.registerAnchor(player.level, "throne_room", player.x, player.y, player.z, player.yaw);
